...
// import { apiPutAgentsBlock } from '../../services/apiService'
import { apiPutAgentEdit } from '../../services/apiService'
import { MdEdit,MdDone,MdClear,MdMenu } from "react-icons/md";
import { Link } from 'react-router-dom';


class AgentMemberList extends React.Component{
  constructor(props) {
    super(props)
    this.state = {
      modifiable:false,
  ...
  // ********** put 修改資料 ********** 
   ...
   if(putData===null || putData==='' || putData===undefined){
      ...
      try{
        let res = await apiPutAgentEdit(this.props.id,putData)
  ...
  render(){
  // console.log('get:remarkGroup ↓',this.state.AgentGroup);
  // let {t} = this.props
  let { modifiable } = this.state
  // console.log('state:',modifiable)
  // console.log('state:',nickname)
  // console.log(this.props.is_block)
      ...
      <tr>
        ...
        <td>
          <button className="detail-btn">
            <Link to={`/member-sell-order-list/${this.props.id}?pageFrom=${this.props.pageId}`}>
              <MdMenu/>
            </Link>
          </button>
        </td>
...
export default withTranslation()(AgentMemberList);