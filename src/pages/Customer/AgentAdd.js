...
import { apiPostAgentAdd } from '../../services/apiService'
...
class AgentAdd extends React.Component{
  ...
  submitPost = async() => {
    ...
    if(this.state.username === ''
      ...
      this.showPasswordNotSame() //兩次輸入的密碼不相同時
    }
    else{
      try {
        let res = await apiPostAgentAdd(postData)
        if(res.data.code === 1){
              ...
              <div className="col-md-10">
                <div className="card card-exchange">
                  ...
                  <div className="header text-center">
                    <h4 className="title">{t('app.agent_add')}</h4>
                  </div>
                ...
                {this.state.showRedirectBtn?
                  <div className="modal-footer">
                    <button className="modal-action-button"
                      onClick={()=>window.location.href="/#/customer/agent-list"}>
...
export default withTranslation()(AgentAdd);