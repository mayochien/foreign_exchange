...
import { DEPOSIT_MAIN_STATUS, HANDLING_FEE_TYPE, CURRENCIES_STR} from '../../const';
...
class MessageListItem extends Component {
  ...
  render() {
    ...
    const { t } = this.props;
    let data_income = this.props.income.toFixed(3)
    let data_outcome = this.props.outcome.toFixed(3)
    ...
    return (
      <React.Fragment>
        <tr className={this.props.status===2?"text-failed":''}>
          ...
          <td className="text-green">{this.showNullLine(data_income)}</td>
          <td className="text-red">{this.showNullLine(data_outcome)}</td>
          <td>{this.props.amount.toFixed(3)}</td>
