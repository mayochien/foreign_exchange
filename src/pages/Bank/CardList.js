...
import { apiGetCardList } from '../../services/apiService'
...
class CardList extends React.Component{
  ...
  getInitialData = async() => { // 初始載入 & 翻頁
    this.showLoading()
    try {
      let res = await apiGetCardList(this.state.activePage,this.state.search_value)
      if(res.data.code===1){
        this.setState({
        ...
        <div className="row">
          <div className="col-md-12">
            <div className="card card-exchange">
              ...
              <h4 className="title">{t('app.card_list')}</h4>
              </div>
